...
load_env_context() {
  captured_env_context_args="$*"
  APP_CODE="f${TARGET_APP_ID}"
  APP_DIR="${ROOT_DIR}/export/${TARGET_ENV}/f${TARGET_APP_ID}"
  APP_CONFIG_FILE="${ROOT_DIR}/conn.json#${TARGET_ENV}"
  APEX_WORKSPACE="DEV"
}
...
if [[ "${captured_env_context_args}" != "${TARGET_ENV} ${TARGET_APP_ID}" ]]; then
  printf 'Expected load_env_context args to be: %s %s\n' "${TARGET_ENV}" "${TARGET_APP_ID}" >&2
...
if [[ "${captured_sql}" != *"apex_application_install.set_application_id(${TARGET_APP_ID});"* ]]; then
  printf 'Expected explicit target app id override in SQL: %s\n' "${captured_sql}" >&2
  exit 1
fi

if [[ "${captured_sql}" != *"@"*"/.filtered_install.sql"* ]]; then
  printf 'Unexpected filtered install invocation: %s\n' "${captured_sql}" >&2
  exit 1
fi

printf 'import_apex_app cross env args test passed\n'
