...
SOURCE_APP_ID="100"
FIRST_CANDIDATE_APP_ID="108"
AUTO_TARGET_APP_ID="109"
...
lookup_target_app_identity() {
  ...
  if [[ "${target_app_id}" == "${FIRST_CANDIDATE_APP_ID}" ]]; then
    printf 'LAB|%s|Occupied Elsewhere\n' "${FIRST_CANDIDATE_APP_ID}"
  fi
}
...
if [[ "${captured_lookup_sql}" != *"where workspace = 'DEV'"* || "${captured_lookup_sql}" != *"application_id = trunc(application_id)"* ]]; then
  printf 'Expected workspace-scoped auto target lookup SQL, got: %s\n' "${captured_lookup_sql}" >&2
  exit 1
fi
...
if [[ "${captured_sql}" != *"apex_application_install.set_application_id(${AUTO_TARGET_APP_ID});"* ]]; then
  printf 'Expected auto target app id to skip conflicts and use %s: %s\n' "${AUTO_TARGET_APP_ID}" "${captured_sql}" >&2
  exit 1
fi

printf 'import_apex_app cross env auto target skip conflict test passed\n'
