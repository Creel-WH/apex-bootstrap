...
RUN_ID="test-import-page-evidence-success"
...
OMX_RUN_ID="${RUN_ID}" bash "${SCRIPT}" ci 100 10

python3 - "${RESULT_DIR}/contract/summary.json" "${RESULT_DIR}/deploy/summary.json" "${RESULT_DIR}/deploy/page.summary.json" <<'PY'
import json
import pathlib
import sys
...
page_path = pathlib.Path(sys.argv[3])
...
assert contract["target"] == "page:100:10->100:10", contract

assert deploy["status"] == "success", deploy
assert deploy["action"] == "deploy-page", deploy

assert page["status"] == "success", page
assert page["action"] == "deploy-page", page
assert page["target"] == "page:100:10->100:10", page
PY

printf 'import_apex_page evidence summary test passed\n'
