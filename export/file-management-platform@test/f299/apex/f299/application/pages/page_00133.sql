...
,p_action=>'NATIVE_JAVASCRIPT_CODE'
,p_attribute_01=>wwv_flow_string.join(wwv_flow_t_varchar2(
'var moveUrl = ''f?p=&APP_ID.:155:&APP_SESSION.::&DEBUG.:155:P155_FILE_IDS:'' + encodeURIComponent($v(''P133_FILE_IDS'') || '''');',
'if (apex.util && apex.util.prepareURL) {',
'    moveUrl = apex.util.prepareURL(moveUrl);',
'}',
'if (apex.navigation && apex.navigation.dialog) {',
unistr('    apex.navigation.dialog(moveUrl, { title: ''\6587\4EF6\8FC1\79FB'', height: ''auto'', width: ''720'', modal: true }, ''t-Dialog-page--standard'');'),
'} else {',
'    apex.navigation.redirect(moveUrl);',
'}'))
);
