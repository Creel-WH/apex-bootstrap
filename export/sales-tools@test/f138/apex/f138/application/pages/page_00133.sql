...
'        apex.item(''P133_PARENT_FOLDER_ID'').setValue(String(objId));',
'        return;',
'    }',
'    $(''#btn_preview'').click();',
'}',
...
'    if (!$bar.length) {',
'        $anchor.before(''<div id="p133_pathbar" class="p133-pathbar"><button type="button" class="p133-up" title="Back to parent"><span class="fa fa-arrow-up" aria-hidden="true"></span></button><div class="p133-crumbs"></div><div class="p133-view-toggle"><button type="button" class="p133-view-btn is-active" data-view-mode="list" title="List view"><span class="fa fa-list" aria-hidden="true"></span></button><button type="button" class="p133-view-btn" data-view-mode="grid" title="Grid view"><span class="fa fa-th-large" aria-hidden="true"></span></button></div><button type="button" class="p133-refresh" title="Refresh"><span class="fa fa-refresh" aria-hidden="true"></span></button></div>'');',
'        $bar = $(''#p133_pathbar'');',
...
'                apex.item(''P133_PARENT_FOLDER_ID'').setValue(upId);',
'            } else {',
'                apex.item(''P133_PARENT_FOLDER_ID'').setValue(rootId);',
'            }',
'        });',
'        $bar.on(''click'', ''.p133-refresh'', function() {',
'            try {',
...
'            if (targetId) {',
'                apex.item(''P133_PARENT_FOLDER_ID'').setValue(String(targetId));',
'            }',
'        });',
'        $bar.on(''click'', ''.p133-view-btn'', function() {',
...
'        views = region ? region.call(''getViews'') : null;',
'        gridView = views ? views.grid : null;',
...
'        var id = p133NormalizeCellValue(model.getValue(record, ''FILE_ID''));',
'        var name = p133NormalizeCellValue(model.getValue(record, ''FILE_NAME''));',
'        var type = p133NormalizeCellValue(model.getValue(record, ''FILE_TYPE''));',
'        var icon = p133NormalizeCellValue(model.getValue(record, ''ICON_CLASS'')) || (type === ''FOLDER'' ? ''fa-folder-o'' : ''fa-file-o'');',
'        cards.push(',
'            ''<button type="button" class="p133-card" data-file-id="'' + apex.util.escapeHTML(String(id || '''')) + ''" data-file-type="'' + apex.util.escapeHTML(String(type || '''')) + ''">'' +',
'            ''<span class="fa '' + apex.util.escapeHTML(String(icon)) + ''" aria-hidden="true"></span>'' +',
'            ''<span class="p133-card-name">'' + apex.util.escapeHTML(String(name || '''')) + ''</span>'' +',
'            ''</button>''',
'        );',
'    });',
...
'    $gridView = $(''#p133_grid_view'');',
'    if ($bar.length) {',
'        $bar.find(''.p133-view-btn'').removeClass(''is-active'');',
'        $bar.find(''.p133-view-btn[data-view-mode="'' + mode + ''"]'').addClass(''is-active'');',
'    }',
'    if (mode === ''grid'') {',
'        $region.hide();',
'        $gridView.show();',
...
'        }, 0);',
'    });',
'    $(document).off(''click.p133'', ''#p133_grid_view .p133-card'').on(''click.p133'', ''#p133_grid_view .p133-card'', function() {',
...
'    gridView = null;',
'}',
'if(gridView && $v(''P133_ROLE_CODE'') ==''VIEW_REPORT''){',
...
'}',
'',
'#p133_pathbar .p133-view-toggle {',
'    display: inline-flex;',
'    align-items: center;',
'    gap: 4px;',
'}',
'',
'#p133_pathbar .p133-view-btn.is-active {',
'    background: #dbeafe;',
'    color: #1d4ed8;',
'}',
'',
...
'       OR (:ROLE_CODE = ''BUSINESS_ADMIN'' AND EXISTS (SELECT 1',
'                                                       FROM STS_SCOPE SB',
...
'ORDER BY CASE WHEN A.FILE_TYPE = ''FOLDER'' THEN 0 ELSE 1 END, LOWER(A.FILE_NAME);'))
