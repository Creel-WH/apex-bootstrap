...
'                success: function (data) {',
'                    if (data && data.code === "200") {',
'                        var currentPageId = String(apex.env.APP_PAGE_ID || "133");',
'                        if (currentPageId === "133" && window.p133ApplyRootState) { $s("P133_PARENT_FOLDER_ID", ""); $s("P133_FILE_ID", ""); $s("P133_FILE_TYPE", ""); p133ApplyRootState(true); if (window.p133RefreshContent) { p133RefreshContent(); } }',
'                        var targetUrl = apex.util.prepareURL("f?p=" + apex.env.APP_ID + ":" + currentPageId + ":" + apex.env.APP_SESSION + ":S::RP," + currentPageId) + "&ts=" + Date.now();',
